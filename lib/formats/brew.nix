...
let
  tarball = import ./tarball.nix {
    inherit pkgs lib deps utils desktop services drv target;
    format = "tar.gz";
    meta = meta // { format = "tar.gz"; };
  };
  ...
  url =
    if meta.downloadUrl != "" then meta.downloadUrl
  ...
  serviceBlock =
    if meta.services == [] || target.os != "darwin" then ""
      ...
      let s = builtins.head meta.services;
          argv = builtins.filter (x: x != "") (lib.splitString " " s.exec);
          rubyArgs = lib.concatMapStringsSep ", "
            (a: ''"${if lib.hasPrefix "/" a then a else a}"'')
            argv;
      in ''
...
pkgs.stdenv.mkDerivation {
  ...
  passthru = { info = meta; inherit target format tarball; };
}
