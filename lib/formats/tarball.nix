...
let
  ...
  ext = format; # "tar.gz" | "tar.xz" | "tar.zst"
  compressor = {
  ...
  prep =
    if isLinux then linuxPrep
    else if isDarwin then darwinPrep
    else if isWindows then windowsPrep
    else darwinPrep;
...
pkgs.stdenv.mkDerivation {
  ...
  dontUnpack = true;
  nativeBuildInputs = with pkgs; [ gnutar gzip xz zstd patchelf file gnugrep rsync coreutils ];
