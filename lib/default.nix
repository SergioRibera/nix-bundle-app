...
let
  ...
  resolveTarget = drv: target:
    if target != null then utils.normalizeTarget target
    else utils.detectTargetFromDrv drv;

  bundle = { drv, format, info ? { }, target ? null }:
    let
      mod = formatModules.${format}
        or (throw "nix-bundle-app: unknown format '${format}'. Supported: ${
          lib.concatStringsSep ", " (builtins.attrNames formatModules)}");
      t = resolveTarget drv target;
      requiredOS = formatOS.${format};
      meta =
        assert lib.assertMsg (requiredOS == "any" || requiredOS == t.os)
          "nix-bundle-app: format '${format}' requires os='${requiredOS}', got os='${t.os}'.";
        infoLib.normalize info drv t format;
    in
      mod {
        inherit pkgs lib deps utils desktop services drv format meta;
        target = t;
      };

  bundleAll = { drv, formats, info ? {}, target ? null }:
    let
      built = map (fmt: {
        name = fmt;
        path = bundle { inherit drv info target; format = fmt; };
      }) formats;
    in
...
in
{
  inherit bundle bundleAll utils desktop services;
